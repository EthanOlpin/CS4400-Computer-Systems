...
int FGPid;                  /* tracks the pid for the FG process */
...
void eval(char *cmdline) 
{
  ...
  if(builtin_cmd(argv1)){
    return; //processed built in command.
  }
  ...
  if(p){
	...
	sio_puts("Failed to unblock SGCHLD signal");
      }
    }
    else{
    ...
    if(bg){
      ...
      do_bg(getjobpid(jobs, getpid())->jid);
    }
    else{
      addjob(jobs, getpid(), FG, cmdline);
      sigprocmask(SIG_UNBLOCK, &set, NULL);
      FGPid = getpid();
      do_fg(getjobpid(jobs, getpid())->jid);
...
void do_bg(int jid) 
{
  ...
  char* argv[MAXARGS];
  parseline(j->cmdline, argv, 1);
  print_job(jid);
  sio_puts(j->cmdline);

  //If job has been stopped, continue it in the bg.
  if(j->state == ST){
    j->state = BG;
    kill(-j->pid, SIGCONT);
    return;
...
void print_job(int jid){
  ...
  sio_putl(jid);
  sio_puts("] (");
  sio_putl(getjobjid(jobs, jid)->pid);
  sio_puts(") ");
}
...
void sigchld_handler(int sig) 
{
  int status;
  int p;
  if((p = waitpid(-1, &status, WNOHANG | WUNTRACED))){
